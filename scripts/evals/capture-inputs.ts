...
const cases = [
  {
    ...
    expected: "Activate Roadmap Review",
  },
  {
    name: "new-project",
    path: "/projects",
    task: "Create and save a new project named Night QA Project.",
    expected: "Activate New project",
  },
  {
    name: "save-ready",
    path: "/item/r-8",
    task: "Save the Roadmap Review document with its current text.",
    expected: "Activate Save document",
  },
  {
    name: "cancel-ready",
    path: "/projects?open=1&draft=Throwaway%20Draft",
    task: "Cancel and discard this new project draft without creating it.",
    expected: "Activate Cancel",
  },
  {
    name: "choice",
    path: "/select-values",
...
try {
  await connection.call({
    name: "browser_tabs",
    arguments: { action: "new", url: workspace.origin },
  });
  ...
  for (const scenario of cases) {
    ...
    await connection.call({
      ...
      arguments: { url: `${workspace.origin}${scenario.path}` },
    });
    // eslint-disable-next-line no-await-in-loop
  ...
  await Bun.write("runs/qa/overnight/decision-inputs.json", JSON.stringify(captured), {
    createPath: true,
  });
  console.log(
    JSON.stringify(
      captured.map(({ name, expected, input }) => ({
        name,
        expected,
        controls: input.observation.window?.elements.length,
