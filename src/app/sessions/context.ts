...
function sessionContext(session: Session): string {
  ...
  return `Previous request: ${JSON.stringify(turn.task.slice(0, REQUEST_CHARS))}\nOutcome: ${turn.status} ${JSON.stringify(turn.message?.slice(0, RESULT_CHARS))}\nPreviously observed: ${JSON.stringify(turn.observation?.slice(0, OBSERVED_CHARS))}\nRecent actions: ${JSON.stringify(turn.actions.slice(-RECENT_ACTIONS))}`;
}
